...
def shortest_path(source, target):
    ...
    target_neighbours = neighbors_for_person(target)
    ...
    while True: 
        ...
        neignbours = neighbors_for_person(parent_node.id) # [(movie_id, person_id)...)
        ...
        for neigbour in neignbours:
            ...
            node = Node(id=neigbour[1], parent=parent_node, movie_id=neigbour[0])
    ...
    if target_node[0].id == target:
        action = []

        parent = target_node[0]

        while True:
            
            if parent.id == source:
                action.reverse()   # requires small touch -- nothing is required
                break

            action.append((parent.movie_id, parent.id))  # in first (movie, target)

            parent_node = parent.parent
        
        # recalibrate action list for path
        path = action
